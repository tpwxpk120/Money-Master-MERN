...
const HomePage = () => {
  ...
  const handleSubmit = async (req, res) => {
    try {
      ...
      console.log(item);
      const response = await fetch(
        "/api/v1/transactions/add-transection",
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
  ...
  const updateSubmit = (req, res) => {
    try {
      ...
      console.log(item);
      fetch("/api/v1/transactions/edit-transection", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
  ...
  useEffect(() => {
    if (type !== "all") {
      ...
      const sortedTransactions = [...allTransaction].sort(
        (a, b) => new Date(b.date) - new Date(a.date)
      );
      setTransection(sortedTransactions);
    }
  }, [allTransaction, type]);
    ...
    <Layout>
      ...
      <div className="filters">
        <div className="filter-tab">
          <h2 id="selectTypeLabel">Select Type</h2>
          <Select
            value={type}
